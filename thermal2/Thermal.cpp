...
void Thermal::sendThermalChangedCallback(const std::vector<Temperature_2_0> &temps) {
    std::lock_guard<std::mutex> _lock(thermal_callback_mutex_);
        std::string val0,val1,st;

    for (auto &t : temps) {
  st=android::hardware::thermal::V2_0::toString(t.throttlingStatus);
     	     std::ifstream file("sys/devices/virtual/thermal/cooling_device0/cur_state");
 		file >> val0;
             file.close();
 if (st=="NONE") {
  	     std::ifstream file1("sys/devices/virtual/thermal/cooling_device1/cur_state");
 		file1 >> val1;
   	     file1.close(); 
} else if (st=="LIGHT") {
	//val0="1800000";
	...
	val1="1310000";
        ...
        std::ofstream file2("/sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq");
        file2 << (val0);
file2.close();
        std::ofstream file6("/sys/devices/system/cpu/cpu4/cpufreq/scaling_max_freq");
        file6 << (val1);
file6.close();
    ...
    if (handle != nullptr && handle->numFds >= 1) {
        ...
        if (!thermal_helper_.isInitializedOk()) {
            ...
            }
            {
             ...
             << android::hardware::thermal::V2_0::toString(t.throttlingStatus)
                         << std::endl;
                        //     std::ofstream file("sys/module/msm_performance/parameters/cpu_max_freq");
                   // std::ofstream file("sdcard/newfile.txt");         
 //  file << "hello from thermal";
